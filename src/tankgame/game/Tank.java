...
public class Tank {
    ...
    private void moveBackwards() {
        ...
        if (Collision.CollidedWith(enemy.getHitBox(), this.getHitBox()))
            velocity.x = -2;
        else
            velocity.x = (int) -Math.round(R * Math.cos(Math.toRadians(angle)));
        collisionAux = velocity;
        collisionAux = Vector2.Add(collisionAux, position);
        Rectangle collV = new Rectangle(collisionAux.x, collisionAux.y, 30, 30);
        if (!CheckAllCollisions(collV)) {
    ...
    private boolean  CheckAllCollisions (Rectangle hitBox) {
        boolean wallCollision = false;
        for (Wall wall : TRE.listWalls) {
            if (wall != null) {
                if (wall.collision_wall && Collision.CollidedWith(hitBox, wall.getHitBox())) {
                    wallCollision = true;
                    break;
                }
            }
